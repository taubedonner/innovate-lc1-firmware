...
/* ------------------------------------------------------------------ */
/*
 * PORTA - вся аналоговая часть контура Нернста.
 *
 * Ток накачки НЕ аналоговый: он фиксирован по величине и переключается
 * только по знаку, а среднее значение задаётся скважностью. Контур целиком
 * представляет собой однобитный сигма-дельта модулятор: ячейка Нернста
 * работает интегратором, аналоговый компаратор - квантователем,
 * DIR_CONTROL - однобитной обратной связью. Поэтому прошивке достаточно
 * измерять T1 и T2: скважность и есть результат.
 *
 * Величину тока задаёт U3B (инвертор с коэффициентом -R8/R9 = -0.605
 * относительно опоры +2.5 В): на его выходе 4.01 В при DIR = 0 и 0.99 В
 * при DIR = 1, то есть перепад +/-1.51 В на резисторе, выбранном ключом U2.
 * Усилитель U1A держит общий электрод датчика LSU_COM на 2.5 В, поэтому
 * весь этот ток течёт через ячейку накачки, а выход U1A (он же LSU_PUMP)
 * измеряется как VPUMP_SENSE.
 */
/* PORTA */
#define P_I_CONTROL \
    0 /*
                               * PA0  выбор тока накачки ПО ТИПУ ДАТЧИКА: ключ U2
                               *      замыкает R6 243 Ом при 1 (Bosch LSU, ~5.5 мА)
                               *      и R5 182 Ом при 0 (NTK, ~7.1 мА). Сбрасывается
                               *      единственный раз - в detect_sensor_type()
                               */
#define P_DIR_CONTROL \
    1 /*
                               * PA1  знак Ipump. Он же выбирает конденсатор
                               *      хранения поправки: 0 -> C14, 1 -> C2
                               */
#define P_RN_MEAS_PULSE \
    2 /*
                               * PA2  импульс нагрузки ячейки Нернста через
                               *      R21 10 кОм прямо в цепь LSU_VN
                               */
#define P_VN_HYST \
    3 /*
                               * PA3  гистерезис компаратора: через R24 8.25 кОм
                               *      сдвигает опору AIN1 на +/-44 мВ вокруг 2.95 В.
                               *      Всегда переключается ВМЕСТЕ с DIR_CONTROL,
                               *      то есть порог шагает вместе с током
                               */
#define P_AC_CENT_ENABLE \
    4 /*
                               * PA4  команда "взять отсчёт" для цепи вычитания
                               *      помехи от тока накачки. Единица замыкает
                               *      ключ U4A, ноль - размыкает (см. nernst_ac.c)
                               */
 ...
 *     80 / 400 / 200. У LSU 4.2 уставка 400 идёт в паре с делителем 5 вместо
 *     25, так что в сопоставимых единицах это те же 80 - паспортное Ri
 *     этого датчика.
 */
...
/* Состояния разбора команд - переменная g_cmd_state (@0x020D), */
enum lc1_cmd_state {
    CMD_IDLE               = 0, /* обычный приём: разбираем очередной байт как команду */
    CMD_RX_NAME            = 1, /* после Listen принимаем 8 байт имени                 */
    CMD_ANSWER_LISTEN_LM1  = 2, /* имя совпало с "LM-1": отвечаем за прибор перед нами */
    CMD_ANSWER_LISTEN_SELF = 3, /* имя наше: отвечаем и уходим в сервисный режим       */
    CMD_ANSWER_TYPELIST    = 4, /* надо ответить на Typelist (0xF3)                    */
    CMD_ANSWER_NAMELIST    = 5, /* надо ответить на Namelist (0xCE)                    */
    CMD_SERVICE            = 6  /* сервисный режим: байты уходят в service_command()   */
};

/*
 * Состояния ретрансляции пакетов - переменная g_relay_state (@0x0219),
 * принятый байт за вызов.
 */
enum lc1_relay_state {
    RELAY_IDLE      = 0, /* ищем начало кадра                                  */
    RELAY_HDR2      = 1, /* принят первый байт заголовка ISP2, ждём второй     */
    RELAY_BODY      = 2, /* пересылаем тело пакета байт за байтом              */
    RELAY_BODY_ISP1 = 3, /* тело пакета LM-1: конвертация ISP1 в ISP2          */
    RELAY_CMD_HI    = 4, /* старший байт слова команды в ответе                */
    RELAY_CMD_LO    = 5, /* младший байт слова команды                         */
    RELAY_NAMES     = 6  /* проход чужих имён в ответе на Namelist             */
};

/*
 * Код функции в пакете протокола - переменная g_status (@0x023E).
 * Набор закрыт спецификацией ISP2: это три бита F2..F0 первого байта
 * включая два, которых в этой прошивке нет. Значение уходит наружу, поэтому
 * менять числа нельзя.
 */
enum lc1_status {
    STATUS_LAMBDA       = 0, /* лямбда достоверна, значение = lambda*1000 */
    STATUS_O2           = 1, /* значение = %O2 в десятых долях процента   */
    STATUS_AIR_CAL      = 2, /* идёт калибровка по свободному воздуху     */
    STATUS_NEED_AIR_CAL = 3, /* требуется калибровка (в образе не ставится)*/
    STATUS_WARMUP       = 4, /* прогрев, значение = % рабочей температуры  */
    STATUS_HEATER_CAL   = 5, /* калибровка нагревателя, значение = отсчёт  */
    STATUS_ERROR        = 6, /* в значении код ошибки                     */
    STATUS_RESERVED     = 7  /* резерв спецификации, не используется      */
};

/*
 * Коды ошибок - переменная g_error_code (@0x0239). Ими же мигает светодиод,
 * и они уходят в протокол при STATUS_ERROR. Источники: heater_resistance_step
 * 3..6; код 8 ставит main, когда автоколебания не запустились повторно.
 */
enum lc1_error {
    ERR_NONE          = 0,
    ERR_RHEAT_LOW     = 1, /* Rheat ниже g_rheat_min_mohm: замыкание нагревателя */
    ERR_NO_HEAT_CURR  = 2, /* ток нагревателя < 10 отсчётов либо Rheat > 30 Ом   */
    ERR_PUMP_SHORT    = 3, /* Rpump < 18                                        */
    ERR_PUMP_OPEN     = 4, /* Rpump >= 600 (у NTK >= 300)                       */
    ERR_NERNST_SHORT  = 5, /* Rnernst < 2                                       */
    ERR_NERNST_OPEN   = 6, /* Rnernst >= 501                                    */
    ERR_SENSOR_TIMING = 8,
    ERR_VBAT_LOW      = 9 /* питание ниже g_vbat_min_raw                       */
};

/*
 * Состояния прибора - переменная g_state (@0x020F), диспетчер
 * Значения заданы самой прошивкой, менять нельзя: они уходят наружу в
 * протокол и используются индикацией (led.c сравнивает с 6 и 8).
 *
 * Порядок прохода при нормальном пуске LSU:
 *   0 -> 1 -> 2 <-> 3 -> 4 -> 5 -> (6 -> 5) -> 10 -> 11 -> 8
 * У NTK ветка иная: 4 -> 10 (если калибровка есть) либо 5 -> 7 -> 4.
 */
enum lc1_state {
    /*
     * Сброс. Гасит аналоговый выход, ждёт конца обмена с ЦАП, читает целевое
     * Rheat из EEPROM[10] (при нуле подставляет 9100/6000/7500 мОм по типу
     * датчика) и задаёт стартовое напряжение нагревателя: 54 единицы для NTK,
     * 433 (~2.1 В) для обоих LSU. Сюда же приводит любая ошибка.
     */
    ST_RESET = 0,

    /*
     * Ожидание первого результата АЦП. Обнуляет запомненное "холодное" Rheat
     * и счётчик выборок. Проходное, задерживается только на флаге данных.
     */
    ST_WAIT_ADC = 1,

    /*
     * Разгон нагревателя. Наращивает запрос эффективного напряжения
     * (g_vheat_ramp) ступенями: у LSU +100 единиц за шаг до 1433 (~7.0 В),
     * дальше +6 до 2660 (~13.0 В); у NTK +1 до 144, а после 144 - только пока
     * прогрев меньше 75 %, и не выше 210. Две скорости соответствуют профилю
     * из паспорта Bosch: быстрый участок ниже 8.5 В, дальше медленный.
     */
    ST_RAMP = 2,

    /*
     * Контроль прогрева. Считает процент прогрева по росту Rheat, фиксирует
     * "холодное" сопротивление как точку 0 %, и когда ячейка достаточно
     * прогрелась (Rnernst ниже порога при прогреве > 65 %), уходит в
     * диагностику. Иначе возвращается в разгон: 2 <-> 3.
     */
    ST_WARMUP = 3,

    /*
     * Диагностика ячеек. Меряет Rpump и Rnernst и выдаёт коды ошибок:
     * 3 - замыкание накачки (<18), 4 - обрыв накачки (>=600, у NTK >=300),
     * 5 - замыкание Нернста (<2), 6 - обрыв Нернста (>=501).
     */
    ST_CELL_CHECK = 4,

    /*
     * Проверка наличия калибровки в EEPROM. У LSU смотрит targetRpump по
     * адресу 0x16: если пусто - на калибровку нагревателя, иначе загружает
     * уставку и идёт раскачивать ячейку. У NTK то же по адресу 10.
     */
    ST_CAL_CHECK = 5,

    /*
     * Калибровка нагревателя (LSU). Держит Rnernst на уставке и в конце
     * запоминает соответствующее ей Rpump. Крутится, пока флаг
     * g_heater_cal_active не снимется.
     */
    ST_HEATER_CAL = 6,

    /*
     * Калибровка нагревателя для NTK. Rnernst не используется: датчик просто
     * греется при фиксированных 2150 единицах (~10.5 В), после чего
     * установившееся Rheat пишется в EEPROM[10].
     */
    ST_HEATER_CAL_NTK = 7,

    /*
     * Рабочий режим. Раз в цикл меряет Rheat, у NTK подруливает нагрев по
     * нему, у LSU температуру держит ПИД по ячейке накачки; раз в три цикла
     * запускает замер Rpump. Только в этом состоянии считается лямбда и
     * работает калибровка по воздуху.
     */
    ST_RUN = 8,

    /*
     * Сброс калибровки. Возвращает целевое Rheat к умолчанию по типу
     * датчика и обнуляет ячейку EEPROM с калибровкой.
     */
    ST_CAL_RESET = 9,

    /*
     * Раскачка ячейки. Компаратор выключен; 50 циклов симметричных импульсов
     * тока накачки с импульсом центрирования AC-связи. Готовит контур к
     * запуску автоколебаний.
     */
    ST_CELL_SHAKE = 10,

    /*
     * Синхронизация фазы. 20 циклов приводят направление тока в противофазу
     * состоянию компаратора, чтобы автоколебания стартовали правильно.
     * Отсюда прибор уходит в рабочий режим.
     */
    ST_AC_SYNC = 11
};
...
/*
 * Второй аргумент report_value_out - что сделать со значением.
 * значение, включая отрицательное, не делает ничего.
 */
enum lc1_aout_mode {
    AOUT_NOW    = 0, /* сбросить таймеры выдачи обоих каналов и накопить  */
    AOUT_ACC    = 1, /* просто накопить, выдача произойдёт по таймеру     */
    AOUT_WARMUP = 2, /* выдать "значение при прогреве", поле out_warmup   */
    AOUT_ERROR  = 3  /* выдать "значение при ошибке",   поле out_error    */
};
...
/*
 * Второй аргумент heater_duty_update - чем делить запрошенное эффективное
 * заставляет функцию выйти, ничего не сделав.
 */
enum lc1_duty_ref {
    DUTY_BY_IHEAT = 0, /* делить на ток нагревателя g_iheat_raw       */
    DUTY_BY_VBAT  = 1  /* делить на напряжение питания g_vbat_scaled  */
};
 ...
 */
/*
 * @0x02FD. Одна ячейка на две роли, так в образе:
 *   - в машине состояний (состояния 0 и 2) это ЗАПРОС эффективного напряжения
 *     нагревателя в единицах 4.883 мВ, который наращивается ступенями, то есть
 *     настоящая рампа разгона;
 *   - в heater_cal_step() и ntk_heater_cal_step() та же ячейка используется
 *     просто как счётчик итераций (до 2000 и до 400 соответственно).
 */
